...
- **ref**: [action.yml](action.yml), `internal/report.Annotations`, [#14](https://github.com/Allan-Nava/robotsmith/issues/14)
...
- **labels**: advise, ux
- **milestone**: v0.4.0 — Your policy, verified continuously
- **ref**: [internal/policy](internal/policy/), `advise.AnalyzeWith`, [#15](https://github.com/Allan-Nava/robotsmith/issues/15)
...
- **labels**: advise
- **milestone**: v0.4.0 — Your policy, verified continuously
- **ref**: [internal/advise/compare.go](internal/advise/compare.go), `report.SnapshotFrom`, [#17](https://github.com/Allan-Nava/robotsmith/issues/17)
...
- **priority**: low
- **labels**: docs, ci
- **ref**: [docs/robots.txt](docs/robots.txt), [docs/sitemap.xml](docs/sitemap.xml), [.github/workflows/dogfood.yml](.github/workflows/dogfood.yml), [dogfood_test.go](dogfood_test.go)

A tool that advises about robots.txt and never looks at its own is an argument nobody has to
believe.

⚠️ **The item's premise was half wrong, and finding out was the useful part.** This is a *project*
page, and crawlers read robots.txt **only from the host root**: a file at
`allan-nava.github.io/robotsmith/robots.txt` governs nothing. The file that speaks for this site is
`allan-nava.github.io/robots.txt`, which lives in another repo and is shared with every project page
on that account. So "ship a robots.txt" could not be the deliverable on its own — that would be
precisely the file that looks right and does nothing, which is what this whole tool is about.

What was delivered instead: the file this project *would* publish if it owned an origin, held to the
tool's own standard by its own tests (clean lint, all 32 expected cases satisfied) and saying out
loud, in its own header, that only the host-root copy is read. Plus `docs/sitemap.xml`, because the
host-root file lists only sitemaps that answer 200 — which is the actual, checkable reason this site
was invisible there. Plus a weekly job that lints the live host-root file (a structural defect there
